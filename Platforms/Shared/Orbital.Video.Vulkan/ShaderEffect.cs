...
namespace Orbital.Video.Vulkan
{
	public sealed class ShaderEffect : ShaderEffectBase
	{
		...
		private static extern IntPtr Orbital_Video_Vulkan_ShaderEffect_Create(IntPtr device);

		[DllImport(Instance.lib, CallingConvention = Instance.callingConvention)]
		private static unsafe extern int Orbital_Video_Vulkan_ShaderEffect_Init(IntPtr handle, IntPtr vs, IntPtr ps, IntPtr hs, IntPtr ds, IntPtr gs, ShaderEffectDesc_NativeInterop* desc);

		[DllImport(Instance.lib, CallingConvention = Instance.callingConvention)]
		private static extern void Orbital_Video_Vulkan_ShaderEffect_Dispose(IntPtr handle);

		public ShaderEffect(Device device)
		: base(device)
		{
			deviceVulkan = device;
			handle = Orbital_Video_Vulkan_ShaderEffect_Create(device.handle);
		}
		...
		protected override bool CreateShader(byte[] data, ShaderType type)
		{
			...
			{
				case ShaderType.VS:
					vs = new Shader(deviceVulkan, type);
					return vs.Init(data);

				case ShaderType.PS:
					ps = new Shader(deviceVulkan, type);
					return ps.Init(data);
		...
		public override void Dispose()
		{
			...
			{
				Orbital_Video_Vulkan_ShaderEffect_Dispose(handle);
				handle = IntPtr.Zero;
			}
