...
extern "C"
{
	...
	ORBITAL_EXPORT int Orbital_Video_D3D12_VertexBuffer_Init(VertexBuffer* handle, void* vertices, uint64_t vertexCount, uint32_t vertexSize, VertexBufferLayout* layout)
	{
		...
		if (vertices != NULL)
		{
			if (handle->mode == VertexBufferMode_GPUOptimized || handle->mode == VertexBufferMode_Read) initialResourceState = D3D12_RESOURCE_STATE_COPY_DEST;// init for gpu copy or CPU read
			else if (handle->mode == VertexBufferMode_Write) initialResourceState = D3D12_RESOURCE_STATE_GENERIC_READ;// init for frequent gpu updates
			else return 0;
		}
		...
		if (vertices != NULL)
		{
			...
			if (useUploadBuffer)
			{
				...
				if (handle->mode != VertexBufferMode_Read)
				{
